...
= Introduction
Over the past decades, the development of Information & Communication Technologies (ICT)—including 5G and fiber to the home (FTTH)—has driven rapid growth in both mobile devices and the mobile applications market. According to International Data Corporation (IDC), more than 1.2 billion mobile devices were in circulation in 2014 @bernardo_silva_analysis_2016. The Grand View Research's mobile market report indicated that the mobile application market reached USD 252.89 billion in 2023 and is projected to grow at a compound annual growth rate (CAGR) of 14.3% from 2024 to 2030, reaching USD 626.39 billion @grand_view_research_mobile_2025. As @fig:size_of_market shows, mobile device operating systems have consolidated from multiple options like Symbian OS @SymbianOS2025 and Windows Phone @WindowsPhone2025 to two main systems: Google Android @AndroidOperatingSystem2025 and Apple iOS @IOS2025. Yet this reduction in operating system choices has not simplified mobile application development.
...
As @fig:type_of_testing shows, The types of mobile application testing can be divided into two categories: functional testing and non-functional testing. Functional testing focuses on verifying the application's functionality, ensuring that it behaves as expected and meets the specified requirements. This includes unit testing, integration testing, system testing, and acceptance testing. Non-functional testing, on the other hand, 
evaluates the application's performance, security, usability, and compatibility across different devices and platforms. This includes performance testing, security testing, usability testing, and compatibility testing. @MobileTestAutomation When in the context of mobile application testing, functional testing is often performed using automated testing tools and frameworks, while non-functional testing may require a combination of automated and manual testing approaches. The choice of testing methods depends on the specific requirements of the application and the goals of the testing process.
...
Compatibility testing ensures the app works across different devices, operating systems, and screen sizes. Automated compatibility tests verify that the application delivers a consistent user experience regardless of the device or platform. @LearnTricentisWhat
...
Along with the pros, there are also some cons of using NightwatchJS:
...
Despite the challenges listed above, some key problems will also be faced when implementing automated mobile testing, they are: 
